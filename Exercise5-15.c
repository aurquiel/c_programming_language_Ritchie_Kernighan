...
int main(int argc, char *argv[])
{
  ...
  int fold = 0;   /*old upper lower case*/
  ...
  {
      my_qsort((void**) lineptr, 0, nlines-1, (int (*)(void*,void*))(numeric ? numcmp : strcmp), reverse ? 1:0, fold ? 1:0);
  ...
  for (i = left+1; i <= right; i++)
      if(reverse)
      {
          if (fold)
          {
              ...
              if (isalpha(*(char*)v[i]) && isalpha(*(char*)v[left]))
              {
                  a=tolower(*(char*)v[i]);
                  b=tolower(*(char*)v[left]);
                  if ((*comp)(&a, &b) > 0)
                      swap(v, ++last, i);
              }else if (isalpha(*(char*)v[i]))
              {
                  a=tolower(*(char*)v[i]);
                  if ((*comp)(&a, v[left]) > 0)
                      swap(v, ++last, i);

              }else if (isalpha(*(char*)v[left]))
              {
                  b=tolower(*(char*)v[left]);
                  if ((*comp)(v[i], &b) > 0)
                      swap(v, ++last, i);
